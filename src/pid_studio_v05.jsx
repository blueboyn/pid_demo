...
import { Edit3, Eye, Plus, Trash2, Code2, Save, X, Move, Layers, GitBranch, FileJson, Download, Upload } from "lucide-react";
...
// ============================================================
// Built-in symbol library (used when no custom geometry from DXF)
// ============================================================
const SYMBOLS = {
...
// ============================================================
// Map shape (from DXF Converter classifier) → built-in symbol type
// ============================================================
const SHAPE_TO_TYPE = {
  rotating: "pump",
  vessel: "tank",
...
// ============================================================
// DXF Block geometry → SVG markup (for customSvg field)
// Replicates DXF Converter's renderer to produce inline SVG
// ============================================================
function dxfEntitiesToSvg(entities, blocks) {
  let svg = "";
  for (const e of entities) {
    svg += entityToSvgString(e);
  }
  return svg;
}

function entityToSvgString(e) {
  switch (e.type) {
    case "LINE":
      return `<line x1="${e.x}" y1="${-e.y}" x2="${e.x2}" y2="${-e.y2}" stroke="#0f172a" stroke-width="0.5" stroke-linecap="round"/>\n`;
    case "CIRCLE":
      return `<circle cx="${e.cx}" cy="${-e.cy}" r="${e.radius}" stroke="#0f172a" stroke-width="0.5" fill="url(#metal)"/>\n`;
    case "ARC": {
      const sa = e.startAngle * Math.PI / 180;
      const ea = e.endAngle * Math.PI / 180;
      const x1 = e.cx + e.radius * Math.cos(sa);
      const y1 = -(e.cy + e.radius * Math.sin(sa));
      const x2 = e.cx + e.radius * Math.cos(ea);
      const y2 = -(e.cy + e.radius * Math.sin(ea));
      let sweep = ea - sa;
      while (sweep < 0) sweep += 2 * Math.PI;
      const largeArc = sweep > Math.PI ? 1 : 0;
      return `<path d="M ${x1} ${y1} A ${e.radius} ${e.radius} 0 ${largeArc} 0 ${x2} ${y2}" stroke="#0f172a" stroke-width="0.5" fill="none"/>\n`;
    }
    case "ELLIPSE": {
      const rx = Math.sqrt(e.majorX ** 2 + e.majorY ** 2);
      const ry = rx * e.ratio;
      const angle = -Math.atan2(e.majorY, e.majorX) * 180 / Math.PI;
      return `<ellipse cx="0" cy="0" rx="${rx}" ry="${ry}" transform="translate(${e.cx} ${-e.cy}) rotate(${angle})" stroke="#0f172a" stroke-width="0.5" fill="url(#dome)"/>\n`;
    }
    case "LWPOLYLINE": {
      ...
      for (let i = 1; i < verts.length; i++) {
        ...
        if (Math.abs(prev.bulge || 0) > 1e-6) {
          ...
          d += ` A ${r} ${r} 0 0 ${prev.bulge > 0 ? 0 : 1} ${v.x} ${-v.y}`;
        } else {
          d += ` L ${v.x} ${-v.y}`;
        }
      }
      if (e.closed) d += " Z";
      return `<path d="${d}" stroke="#0f172a" stroke-width="0.5" fill="${e.closed ? 'url(#metal)' : 'none'}" stroke-linejoin="round"/>\n`;
    }
    default: return "";
  }
}
...
// ============================================================
// Animated overlays per type (unchanged from v0.4)
// ============================================================
const AnimatedOverlay = ({ type, s, c, id }) => {
  switch (type) {
    case "tank": {
      ...
      return (
        <>
          <clipPath id={`tankClip-${id}`}><rect x="-26" y="-38" width="52" height="76"/></clipPath>
      ...
      return (
        <>
          ...
          <g>
            ...
            {[0, 120, 240].map(a => (
              <path key={a} d="M 4 -1.5 Q 11 -2.5 13 0 Q 11 2.5 4 1.5 Z"
...
// ============================================================
// JSON Import — convert DXF Converter output to PID Studio state
// ============================================================
function importFromDxfConverter(json) {
  const symbols = [];
  const pipes = [];
  let nextSymbolId = 1;
  let nextPipeId = 1000;

  // Convert instances → symbols (with rich custom SVG from DXF blocks)
  if (json.instances) {
    for (const inst of json.instances) {
      const blockEntities = json.blocks?.[inst.blockName] || [];
      const customSvg = blockEntities.length > 0 ? dxfEntitiesToSvg(blockEntities, json.blocks) : null;
      const type = SHAPE_TO_TYPE[inst.shape] || "pump";
      symbols.push({
        id: nextSymbolId++,
        type,
        x: inst.x,
        y: inst.y,
        scale: inst.scale || 1,
        tag: inst.tag || `${inst.blockName}-${inst.id}`,
        customSvg,
        shape: inst.shape,
  ...
  return {
    ...
    metadata: { id: json.id, source: json.source, importedAt: new Date().toISOString() },
  };
}

function exportToJson(symbols, pipes, metadata = {}) {
  return {
    id: metadata.id || `PID-${Date.now()}`,
    name: metadata.name || "Untitled",
    version: 2,
    createdAt: metadata.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    symbols: symbols.map(s => ({
      id: s.id, type: s.type, x: s.x, y: s.y, scale: s.scale,
      tag: s.tag, customSvg: s.customSvg, shape: s.shape,
...
export default function PIDStudio() {
  ...
  const [viewBox, setViewBox] = useState("0 0 700 360");
  const svgRef = useRef(null);
  const nextSymbolId = useRef(200);
  const nextPipeId = useRef(2000);
  ...
  const handleJsonImport = async (file) => {
    ...
    try {
      ...
      // Detect format: DXF Converter output (has `instances` + `blocks`) vs PID Studio save (has `symbols` directly)
      let imported;
      if (json.instances || json.blocks) {
        // DXF Converter format
        imported = importFromDxfConverter(json);
        // Adjust viewport to nice canvas coords (DXF coords are arbitrary)
        if (imported.viewport) {
          const v = imported.viewport;
          const pad = Math.max(v.w, v.h) * 0.1;
          setViewBox(`${v.x - pad} ${v.y - pad} ${v.w + 2 * pad} ${v.h + 2 * pad}`);
        }
      } else if (json.symbols) {
        // Native PID Studio format
        imported = { symbols: json.symbols, pipes: json.pipes || [],
                     metadata: { id: json.id, name: json.name } };
      } else {
        alert("인식할 수 없는 JSON 형식입니다.");
        return;
      }

      setSymbols(imported.symbols);
      setPipes(imported.pipes);
      setProjectName(file.name.replace(/\.(json|pid\.json)$/, ""));
      // Update id counters
      const maxSymId = Math.max(0, ...imported.symbols.map(s => s.id));
      const maxPipeId = Math.max(0, ...imported.pipes.map(p => p.id));
      nextSymbolId.current = maxSymId + 1;
      nextPipeId.current = maxPipeId + 1;
      setSelection(null);
      setImportFlash("imported");
  ...
  const handleJsonExport = () => {
    const data = exportToJson(symbols, pipes, { name: projectName });
    downloadJSON(data, `${projectName}.pid.json`);
    setImportFlash("exported");
    setTimeout(() => setImportFlash(null), 2000);
  };
      ...
      <header className="border-b border-slate-800/60 px-6 py-3 flex items-center gap-4">
        <div>
          <div style={{ fontFamily: '"Instrument Serif", serif' }} className="text-xl italic text-slate-100 leading-none">
            P&ID Studio <span className="text-amber-400">·</span> v0.5
          </div>
          <div className="text-[9px] uppercase tracking-[0.2em] text-slate-500 mt-1 font-mono">
            edit · visualize · import/export
          </div>
        </div>

        {/* Project name */}
        <input type="text" value={projectName} onChange={e => setProjectName(e.target.value)}
               className="px-2 py-1 text-sm bg-slate-900 border border-slate-700 rounded font-mono text-slate-300 w-40 focus:border-amber-500 outline-none"
               placeholder="project name"/>

        <div className="ml-auto flex items-center gap-2">
          {/* JSON I/O */}
          <label className="cursor-pointer flex items-center gap-2 px-3 py-1.5 rounded-md border border-cyan-500/40 hover:bg-cyan-500/10 text-sm text-cyan-300">
            <Upload size={14}/> JSON 열기
            <input type="file" accept=".json" className="hidden"
                   onChange={e => handleJsonImport(e.target.files?.[0])}/>
          </label>
          <button onClick={handleJsonExport}
                  className="px-3 py-1.5 rounded-md border border-amber-500/40 hover:bg-amber-500/10 text-sm text-amber-300 flex items-center gap-2">
            <Download size={14}/> JSON 저장
          </button>

          {/* Mode toggle */}
          <div className="flex border border-slate-700 rounded-md overflow-hidden ml-2">
      ...
      <div className="flex-1 flex overflow-hidden">
              ...
              <div>
                <button onClick={drawingPipe ? cancelDrawingPipe : startDrawingPipe}
                        className={`w-full px-3 py-2 rounded-md text-sm flex items-center justify-center gap-2 border transition-all ${
                          drawingPipe ? "border-rose-500/50 bg-rose-500/10 text-rose-300"
                ...
                {drawingPipe && (
                  <div className="mt-2 text-[10px] text-amber-400 font-mono leading-relaxed">
                    {drawingPipe === "armed" ? "캔버스에서 시작점 클릭" : "끝점을 클릭하여 완성"}
                  </div>
                )}
              </div>
                ...
                <div className="space-y-0.5 max-h-72 overflow-y-auto">
                  {symbols.map(sy => (
                    <button key={`s-${sy.id}`} onClick={() => setSelection({ type: "symbol", id: sy.id })}
                      ...
                      <span className="text-slate-200 truncate">{sy.tag}</span>
                      <span className="text-slate-500 text-[10px]">{SYMBOLS[sy.type]?.name || sy.type}</span>
                    </button>
                  ))}
                  {pipes.map(p => (
        ...
        <main className="flex-1 relative overflow-hidden">
          ...
          <div className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1.5 bg-slate-900/80 backdrop-blur rounded-md border border-slate-800 font-mono text-[11px]">
            ...
            <span className="text-slate-300">{mode === "edit" ? "EDIT" : "VIZ"}</span>
          ...
          {importFlash && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 bg-slate-900 border border-cyan-500/40 rounded-md text-sm text-cyan-300 font-mono">
              ✓ {importFlash === "imported" ? "JSON 가져오기 완료" : "JSON 저장 완료"}
            </div>
          )}
        </main>
          ...
          <aside className="w-72 border-l border-slate-800/60 bg-slate-950/40 p-4 overflow-y-auto">
            {selectedSymbol ? (
              <>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    {selectedSymbol.shape && (
                      <div className="text-[10px] text-cyan-400/70 font-mono mt-1">shape: {selectedSymbol.shape}</div>
                    )}
                  </div>
                ...
                <Field label="실시간 바인딩 태그" value={selectedSymbol.binding || ""}
                       onChange={v => updateSelectedSymbol({ binding: v || null })}/>
